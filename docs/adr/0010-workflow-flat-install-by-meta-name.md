...
- Script có `agentType` dạng `<plugin>:<agent>` là gắn với plugin. Nó vẫn có trong Danh mục nguồn nhưng không được cài: chọn đích danh thì là xung đột, nằm trong "tất cả" thì bị bỏ qua kèm thông báo. `agentType` không tiền tố hoặc `workflow('<name>')` trỏ tới tên không có ở Scope và không được khai báo thì chỉ cảnh báo, không tự cài. Agent có sẵn của Claude Code không tính là thiếu.
