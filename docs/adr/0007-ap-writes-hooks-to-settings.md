...
Hook là code tuỳ ý chạy với quyền người dùng, kể cả không qua hộp thoại tin cậy ở chế độ `-p`. Vì vậy hook đến từ Remote preset phải được xác nhận khi mới thêm hoặc đổi nội dung; chạy không tương tác thì cần `--yes`. Cùng lý do với việc [ADR 0006](0006-mcp-servers-inline-plus-bundled-catalog.md) không tự duyệt `.mcp.json`.
...
- Cấm hook trong Remote preset — bị loại vì quá chặt; xác nhận khi nội dung đổi là đủ vì Remote preset đã ghim theo nội dung.
...
- Hook không có định danh trong settings, nên `ap` nhận ra Installed hook của mình bằng nội dung: mỗi Hook declaration là một nhóm matcher riêng, Lock/State lưu tên → nguyên nhóm đã chuẩn hoá (như `ManagedMcp` của ADR 0006). Không gắn khoá lạ kiểu `x-ap` vào settings, vì tài liệu không bảo đảm và schema SchemaStore từ chối. Nhóm bị sửa tay (không còn nhóm nào khớp) là xung đột theo [ADR 0003](0003-managed-entries-per-scope-state.md), chỉ ghi đè khi có `--force`.
...
- Hook declaration dùng `${CLAUDE_PROJECT_DIR}` hoặc đường dẫn tương đối mà nhắm scope `user` → cảnh báo (hook chạy ở mọi project, thiếu file thì lỗi nhưng không chặn); không chặn vì có người cố ý dùng.
...
- Xác nhận áp dụng cho Hook declaration có nguồn gốc (Preset định nghĩa nó sau cùng khi ghi đè) là Remote preset và nội dung chưa có trong Lock/State. Không có TTY và thiếu `--yes` → bỏ qua các hook đó, cảnh báo, exit code khác 0; phần còn lại vẫn sync.
