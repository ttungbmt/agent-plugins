...
- **Bundled preset nằm trong `packages/cli/presets/`, được commit.** `defaultPresetsDir()` chỉ đọc thư mục đó, nên dev và release đi cùng một đường; `pack-release.mjs` copy nguyên thư mục vào tarball. Trước đây preset ở package `presets` riêng và mọi bước build copy sang đây (gitignore): bản copy dễ cũ khi chạy test mà chưa build, và có hai chỗ trùng tên file để sửa nhầm, nên đã bỏ.
- **Tên:** package `packages/cli` là `@ttungbmt/agent-plugins`, lệnh vẫn là `ap`; package gốc đổi thành `agent-plugins-monorepo`, `private: true`. Ban đầu là `agent-plugins` không scope; đổi sang scope của username sau `v0.2.0` để giữ sẵn đường phát hành lên npm registry sau này (tên không scope có thể đã bị người khác chiếm, còn scope `@ttungbmt` chỉ chủ tài khoản publish được).
